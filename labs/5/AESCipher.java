/*
 * file: AESCipher.java
 * author: Harsha Kota
 * course: MSCS 630
 * assignment: Lab 5
 * due date: Wednesday, April 18, 2018
 * version: 1.0
 *
 * This file contains the declaration of the
 * AESCipher class.
 */

import java.util.ArrayList;

/*
 * AESCipher
 *
 * This class implements the Advanced Encryption Standard (AES) algorithm to produce an Encrypted message
 */
public class AESCipher {

  // Rijndael S-box
  private static final char[] SBOX =
    {
      
    };

  // Rcon - Rijndael key schedule
  private static final char[] RCON =
    {
      
    };

  // Byte multiplication by 2
  private static final char[] MUlBY2 =
    {
      
    };

  // Byte multiplication by 3
  private static final char[] MUlBY3 =
    {
      
    };

  // Fixed circulant matrix for multiplications in Galois Fields for step: MixColumns
  private static final int[][] RMIXCOLUMNS =
    {
      {2, 3, 1, 1},
      {1, 2, 3, 1},
      {1, 1, 2, 3},
      {3, 1, 1, 2}
    };

  /*
   * aesSBox
   *
   * This function takes an input character and provides a substitution char using
   * the Rijndael S-box
   *
   * Parameters:
   *   inHex:      Char to be transformed using the Rijndael S-box
   *   it:         Integer value of the character, used as an index on Rijndael S-box
   *
   * Return value: Transformed character
   */
  private static char aesSBox(char inHex) {
    int it = (int) inHex;
    return SBOX[it];
  }

  /*
   * aesRcon
   *
   * This function takes an input integer and provides a constant from Rcon
   *
   * Parameters:
   *   round:       Integer value of the round, used as an index on RCON
   *
   * Return value:  Character from RCON for that round
   */
  private static char aesRcon(int round) {
    return RCON[round];
  }

  /*
   * aesRoundKeys
   *
   * This function performs AES specific transformations to the input 128-bit key
   * to generate 10 additional round keys, one on each round.
   *
   * Parameters:
   *   noOfKeys:      Original key + 10 rounds keys
   *   temp_key:      Hexadecimal values as string, split from the 128-bit key as a pair of 2 values
   *   key[]:         Holds the 128-bit key as an array of 16 ASCII character values
   *   W[][]:         4x44 matrix, since each key is made up of 16 ASCII values, each key making up a 4x4 matrix of W.
   *   answer:        Holds all round keys as strings
   *   temp_answer:   Temporary array to hold the key values as they are being build
   *   col:           Counter for the column index
   *   counter:       Integer counter as an index to traverse through each value in the key[]
   *   temp:          Holds W[col-1]
   *   firstItem:     Holds the first value from the temp[]
   *   temp_new:      Holds the transformed values from the aesSBox function for values in temp[]
   *   Rcon:          Holds the Round constant for the round
   *   temp_String:   Temporary string builder to build the key using Array W for every 4x4 row&column, going by column
   *
   *
   *
   *
   * Return value: String array of round keys including the original key
   */
  private static String[] aesRoundKeys(String KeyHex) {

    int noOfKeys = 11;

    // Splits the Key String to 16 ASCII Hexadecimal string values (pair of 2)
    // Parses Hexadecimal string to ASCII character value and stores them in a char array
    String[] temp_key = KeyHex.split("(?<=\\G.{2})");
    char[] key = new char[temp_key.length];
    for (int i=0; i<temp_key.length; i++) {
      key[i] = (char) Integer.parseInt(temp_key[i], 16);
    }

    char[][] W = new char[4][44];

    String[] answer = new String[noOfKeys];
    ArrayList<String> temp_answer = new ArrayList<>();

    int col = 0;

    // Step 1
    // W(0) = key(0), W(1) = key(1), W(2) = key(2), and W(3) = key(3)
    int counter = 0;
    for (int c=0; c<4; c++){
      for (int r=0; r<4; r++) {
        W[r][c] = key[counter];
        counter++;
      }
      col++;
    }

    /* Step 2
     * Perform various transformations using the given key in W(0), W(1), W(2), W(3)
     * to produce the round keys in the other 40 columns of W
     */
    while (col < W[0].length){

      // Part B
      // If the index of the column (col) is a multiple of 4, it indicates its a new round. Making every 4x4 matrix,
      // a key of size 16 in ASCII.
      if (col%4 == 0) {

        // Part I
        // For the construction of W[col], we will use the elements from the previous column W[col-1]
        char[] temp = new char[4];
        for (int r=0; r<4; r++) {
          temp[r] = W[r][col-1];
        }

        // Part II
        // Left Shift rows as follows: Wnew = [ W[1][col-1], W[2][col-1], W[3][col-1], W[0][col-1] ]
        char firstItem = temp[0];
        System.arraycopy(temp, 1, temp, 0, 3);
        temp[3] = firstItem;

        // Part III
        // Transform each of the values in Wnew using the SBOX
        char[] temp_new = new char[4];
        for (int r=0; r<4; r++) {
          temp_new[r] = aesSBox(temp[r]);
        }

        // Part IV
        // Get the Rcon round constant for the round. (Current round = col/4)
        char Rcon = aesRcon(col/4);

        // Part V
        // Perform an XOR operation on the first value in Wnew using the Rcon for the round
        temp_new[0] = (char) (Rcon ^ temp_new[0]);

        // Part VI
        // W[col] = W[col-4] XOR Wnew
        for (int r=0; r<4; r++) {
          W[r][col] = (char) (W[r][col-4] ^ temp_new[r]);
        }

        col++;

      } else {

        // Part A
        // If the index of the column (col) is not a multiple of 4. We XOR the fourth past and last column
        // with respect to col, W[col] = W[col-4] XOR W[col-1]
        for (int r=0; r<4; r++) {
          W[r][col] = (char) (W[r][col-4] ^ W[r][col-1]);
        }

        col++;

      }

    }

    StringBuilder temp_String = new StringBuilder();

    // Traverse through W by column converting each value to a 2-digit Hexadecimal Uppercase value.
    // After every 4th column, key is saved in temp_answer
    for (int c=0; c<W[0].length; c++) {
      if (c%4 != 0 || c==0) {
        for (int r=0; r<4; r++) {
          temp_String.append(String.format("%02X", (int) W[r][c]));
        }
      } else {
        temp_answer.add(temp_String.toString());
        temp_String.setLength(0);
        for (int r=0; r<4; r++) {
          temp_String.append(String.format("%02X", (int) W[r][c]));
        }
      }
    }

    // To save the last round key
    temp_answer.add(temp_String.toString());
    temp_String.setLength(0);

    // Transfer all keys to a String[] in UpperCase
    for (int i=0; i<answer.length; i++) {
      answer[i] = temp_answer.get(i);
    }

    return answer;
  }

  /*
   * aesStateXOR
   *
   * This function performs "Add Round Key" operation, which is XOR of the corresponding input matrix entries,
   * State Matrix and Round Key to generate a new State Matrix.
   *
   * Parameters:
   *   sHex:              Input State Matrix
   *   keyHex:            Input Round Key
   *   resultState[][]:   Will hold the transformed matrix from the XOR operation of sHex and keyHex
   *   temp_StringKey:    Hexadecimal values as string, split from the 128-bit key as a pair of 2 values
   *   temp_CharKey:      Holds the 128-bit key as an array of 16 ASCII character values
   *   counter:           Integer counter used as an index to traverse through each value in the key[]
   *   key[][]:           Holds the key, ASCII character values in a 4x4 matrix
   *
   * Return value: New State Matrix of size 4x4
   */
  private static char[][] aesStateXOR(char[][] sHex, String keyHex) {

    char[][] resultState = new char[4][4];

    String[] temp_StringKey = keyHex.split("(?<=\\G.{2})");
    char[] temp_CharKey = new char[temp_StringKey.length];
    for (int i=0; i<temp_StringKey.length; i++) {
      temp_CharKey[i] = (char) Integer.parseInt(temp_StringKey[i], 16);
    }
    int counter=0;
    char[][] key = new char[4][4];
    for (int c=0; c<4; c++) {
      for (int r=0; r<4; r++) {
        key[r][c] = temp_CharKey[counter];
        counter++;
      }
    }


    for (int r=0; r<4; r++) {
      for (int c=0; c<4; c++) {
        resultState[r][c] = (char) (sHex[r][c] ^ key[r][c]);
      }
    }

    return resultState;
  }

  /*
   * aesNibbleSub
   *
   * This function performs "Substitution" operation, i.e., the entries of the input State Matrix are run through
   * aesSBox function which provides a substitution char using Rijndael S-box
   *
   * Parameters:
   *   inStateHex:        Input State Matrix
   *   resultState[][]:   Will hold the transformed matrix after the Substitution operation
   *
   * Return value: New State Matrix of size 4x4
   */
  private static char[][] aesNibbleSub(char[][] inStateHex) {

    char[][] resultState = new char[4][4];

    for (int r=0; r<4; r++) {
      for (int c=0; c<4; c++) {
        resultState[r][c] = aesSBox(inStateHex[r][c]);
      }
    }

    return resultState;
  }

  /*
   * aesShiftRow
   *
   * This function performs "Shift Rows" operation, the State Matrix four rows are shifted cyclically to the left by
   * offsets of 0, 1, 2, 3, resulting in a new State Matrix
   *
   * Parameters:
   *   inStateHex:        Input State Matrix
   *   resultState[][]:   Will hold the transformed matrix after the Shift Rows operation
   *   row:               Holds and a single row and is used to perform the shift using arraycopy methods
   *   tempHolder:        Holds r number of values for row r starting at head, these values are appended to the end
   *                      of the shifted row variable completing the shift
   *
   * Return value: New State Matrix of size 4x4
   */
  private static char[][] aesShiftRow(char[][] inStateHex) {

    char[][] resultState = new char[4][4];

    // First row is offset by 0, so no need to shift
    for (int r=0; r<1; r++){
      for (int c=0; c<4; c++) {
        resultState[r][c] = inStateHex[r][c];
      }
    }

    // Shifting rows 2, 3, 4 with offsets 1, 2, 3 respectively
    for (int r=1; r<4; r++) {
      char[] row = new char[4];
      char[] tempHolder = new char[r];
      for (int c=0; c<r; c++) {
        tempHolder[c] = inStateHex[r][c];
      }
      System.arraycopy(inStateHex[r], r, row, 0, inStateHex[r].length - tempHolder.length);
      System.arraycopy(tempHolder, 0, row, inStateHex[r].length - tempHolder.length, tempHolder.length);
      System.arraycopy(row, 0, resultState[r], 0, row.length);
    }

    return resultState;
  }

  /*
   * GaloisMul
   *
   * This function aids aesMixColumn by performing Galois Multiplication on the values of State Matrix the Fixed Matrix
   * by the use of lookup tables for byte multiplication by 1, 2, 3
   *
   * Parameters:
   *   c:                 Input value from the State Matrix
   *   b:                 Corresponding Input value from the Fixed Matrix
   *
   * Return value: Transformed char value for corresponding value in State Matrix and Fixed Matrix
   */
  private static char GaloisMul(char c, int b) {
    if (b == 2) {
      return MUlBY2[(int)c];
    }
    if (b == 3) {
      return MUlBY3[(int)c];
    }

    return c;
  }

  /*
   * GaloisMul
   *
   * This function performs "Mix Column" operation, i.e., Galois Multiplication on the values of State Matrix and
   * corresponding values in a Fixed Matrix with the use GaloisMul method resulting a new State Matrix
   *
   * Parameters:
   *   inStateHex:        Input State Matrix
   *   resultState[][]:   Will hold the transformed matrix after the Mix Column operation
   *
   * Return value: New State Matrix of size 4x4
   */
  private static char[][] aesMixColumn(char[][] inStateHex) {

    char[][] resultState = new char[4][4];

    for (int c=0; c<4; c++) {
      for (int r=0; r<4; r++) {
        resultState[r][c] = (char)
                (GaloisMul(inStateHex[0][c], RMIXCOLUMNS[r][0]) ^ GaloisMul(inStateHex[1][c], RMIXCOLUMNS[r][1]) ^
                GaloisMul(inStateHex[2][c], RMIXCOLUMNS[r][2]) ^ GaloisMul(inStateHex[3][c], RMIXCOLUMNS[r][3]));
      }
    }

    return resultState;
  }

  /*
   * AES
   *
   * This function performs AES encryption by performing specific operations on the Input plainText using a 128-bit key
   * and 10 additional round keys generated from the aesRoundKeys method using the original key, resulting in an
   * encrypted message
   *
   * Parameters:
   *   pTextHex:          Input plain text in hexadecimal format
   *   keyHex:            Encryption key in hecadecimal format
   *   result:            Holds the encrypted messages as its being built
   *   temp_pText:        Hexadecimal values as string, split from the 128-bit plain text as a pair of 2 values
   *   pText:             Holds the 128-bit key as an array of 16 ASCII character values
   *   stateMatrix[][]:   Will hold the plain text in a matrix of 4x4
   *   counter:           Integer counter used as an index to traverse through each value in the pText[]
   *   keys:              Holds all the keys, i.e., original key and 10 additional round keys
   *   roundCounter:      Keeps track of current column
   *   temp_stateMatrix:  Holds the transformed State Matrix after each operation
   *
   * Return value: Encrypted Message as a hexadecimal string
   */
  public static String AES(String pTextHex, String keyHex) {

    StringBuilder result = new StringBuilder();

    String[] temp_pText = pTextHex.split("(?<=\\G.{2})");
    char[] pText = new char[temp_pText.length];
    for (int i=0; i<temp_pText.length; i++) {
      pText[i] = (char) Integer.parseInt(temp_pText[i], 16);
    }

    char[][] stateMatrix = new char[4][4];

    int counter = 0;
    for (int c=0; c<4; c++){
      for (int r=0; r<4; r++) {
        stateMatrix[r][c] = pText[counter];
        counter++;
      }
    }

    String[] keys = aesRoundKeys(keyHex);

    int roundCounter = 0;

    char[][] temp_stateMatrix = new char[4][4];
    while (roundCounter < 11) {

      if (roundCounter == 0) {
        temp_stateMatrix = aesStateXOR(stateMatrix, keys[roundCounter]);
        roundCounter++;
      }

      if (roundCounter < 10) {
        temp_stateMatrix = aesNibbleSub(temp_stateMatrix);
        temp_stateMatrix = aesShiftRow(temp_stateMatrix);
        temp_stateMatrix = aesMixColumn(temp_stateMatrix);
        temp_stateMatrix = aesStateXOR(temp_stateMatrix, keys[roundCounter]);
        roundCounter++;
      } else {
        temp_stateMatrix = aesNibbleSub(temp_stateMatrix);
        temp_stateMatrix = aesShiftRow(temp_stateMatrix);
        temp_stateMatrix = aesStateXOR(temp_stateMatrix, keys[roundCounter]);
        roundCounter++;
      }
    }

    for (int c=0; c<4; c++) {
      for (int r=0; r<4; r++) {
        result.append(String.format("%02X", (int) temp_stateMatrix[r][c]));
      }
    }

    return result.toString();
  }

}
